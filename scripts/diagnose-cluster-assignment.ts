...
async function diagnoseClusterAssignment(conversationId: string) {
  ...
  const { data: responses, error } = await supabase
    ...
    .select('id, response_text, cluster_index, x_umap, y_umap, is_misc, distance_to_centroid, outlier_score')
  ...
  if (freedomResponse) {
    ...
    console.log(`   Position: (${freedomResponse.x_umap?.toFixed(2)}, ${freedomResponse.y_umap?.toFixed(2)})`);
    console.log(`   Distance to Centroid: ${freedomResponse.distance_to_centroid?.toFixed(4)}`);
    console.log(`   Outlier Score: ${freedomResponse.outlier_score?.toFixed(2) || 'N/A'}`);
    console.log('');
  } else {
  ...
  console.log(`📝 Found ${testResponses.length} responses containing "test":\n`);
  ...
  for (const [clusterIdx, resps] of testByCluster.entries()) {
    ...
    console.log(`Cluster ${clusterIdx}: ${theme?.name || 'Unknown'} (${resps.length} responses)`);
    for (const r of resps.slice(0, 5)) { // Show first 5
      console.log(`   - "${r.response_text.substring(0, 60)}${r.response_text.length > 60 ? '...' : ''}"`);
    }
    if (resps.length > 5) {
  ...
  for (const r of responses) {
    clusterCounts.set(r.cluster_index, (clusterCounts.get(r.cluster_index) || 0) + 1);
  }
  ...
  if (freedomResponse) {
    ...
    if (freedomClusters.size > 1) {
      console.log(`🔴 "Freedom" related responses split across ${freedomClusters.size} clusters:`);
      for (const clusterIdx of freedomClusters) {
        const theme = themeByCluster.get(clusterIdx);
        const count = freedomRelated.filter(r => r.cluster_index === clusterIdx).length;
        console.log(`   Cluster ${clusterIdx} (${theme?.name}): ${count} responses`);
      }
      console.log('');
    }
  }
  ...
  if (testByCluster.size > 1) {
    console.log(`🔴 "Test" related responses split across ${testByCluster.size} clusters:`);
    for (const [clusterIdx, resps] of testByCluster.entries()) {
      const theme = themeByCluster.get(clusterIdx);
      console.log(`   Cluster ${clusterIdx} (${theme?.name}): ${resps.length} responses`);
    }
    console.log('');
  }
}
...
if (!conversationId) {
  console.error('Usage: npx tsx scripts/diagnose-cluster-assignment.ts <conversation-id>');
