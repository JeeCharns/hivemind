...
describe("kmeans clustering", () => {
  ...
  describe("knee detection", () => {
    it("detects natural clusters in clearly separated data", () => {
      ...
      const cluster1 = Array(50).fill(null).map((_, i) => [
        0 + (i % 3) * 0.1,
        0 + (i % 2) * 0.1,
        0 + (i % 4) * 0.1,
      ]);
      const cluster2 = Array(50).fill(null).map((_, i) => [
        100 + (i % 3) * 0.1,
        100 + (i % 2) * 0.1,
        100 + (i % 4) * 0.1,
      ]);
      ...
      const result = clusterEmbeddings(embeddings, undefined, { minClusterSize: 8 });
    ...
    it("finds multiple clusters in diverse data", () => {
      ...
      const cluster1 = Array(15).fill(null).map((_, i) => [
        0 + (i % 3) * 0.01,
        0 + (i % 2) * 0.01,
      ]);
      const cluster2 = Array(15).fill(null).map((_, i) => [
        10 + (i % 3) * 0.01,
        0 + (i % 2) * 0.01,
      ]);
      const cluster3 = Array(15).fill(null).map((_, i) => [
        5 + (i % 3) * 0.01,
        10 + (i % 2) * 0.01,
      ]);
    ...
    it("does not force k=2 on data that warrants k=1", () => {
      ...
      const embeddings = Array(30).fill(null).map((_, i) => [i, i, i]);
  ...
  describe("adaptive max k", () => {
    it("respects minClusterSize constraint", () => {
      ...
      const embeddings = Array(24).fill(null).map((_, i) => [i, i * 2]);
    ...
    it("respects n/3 safety bound", () => {
      ...
      const embeddings = Array(15).fill(null).map((_, i) => [i, i * 2]);
    ...
    it("allows manual maxClusters override within bounds", () => {
      ...
      const embeddings = Array(30).fill(null).map((_, i) => [i, i * 2]);

      const result = clusterEmbeddings(embeddings, undefined, { maxClusters: 2 });
    ...
    it("handles large datasets without forcing small k", () => {
      ...
      const embeddings = Array(200).fill(null).map((_, i) => [
        Math.sin(i / 10),
        Math.cos(i / 10),
        i % 5,
      ]);
  ...
  describe("manual k override", () => {
    ...
    it("handles manual k=1", () => {
      const embeddings = Array(20).fill(null).map((_, i) => [i, i * 2]);
  ...
  describe("deterministic behavior", () => {
    it("produces consistent results with seed", () => {
      const embeddings = Array(50).fill(null).map((_, i) => [
        Math.sin(i),
        Math.cos(i),
      ]);
