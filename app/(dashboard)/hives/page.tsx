...
export default function HivesPage() {
  ...
  const phaseToTab = (phase: string) => {
    ...
    if (normalized.includes("report") || normalized.includes("review"))
      return "report";
    if (normalized.includes("understand") || normalized.includes("synthesis"))
      return "understand";
    return "listen";
  };
  ...
  const handleOpenSession = (session: Session) => {
    ...
    const tab = phaseToTab(session.currentPhase);
    router.push(`/hives/${session.id}/conversations/${session.id}/${tab}`);
  };
